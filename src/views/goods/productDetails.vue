...
<template>
  <div>
    ...
    <div
      style="background-color: #fff; width=100%; height=60px; padding-top: 2px; padding-bottom:10px; padding-left:20px;"
    >
      <div style=" width=50px; float:left;">
        <img
          :src="img_url + goodsInfo.avatarUrl"
          alt="头像"
    ...
    <div id="myGoods">
      ...
      <div id="goodsInfo">
        <van-cell-group inset>
          <van-cell id="goodsPrice" :border="false">
            <template #default>
              <van-row>
                <!--            商品名-->
                <van-col span="24" class="goodsName">{{
                  goodsInfo.name
                }}</van-col>
              </van-row>
              <van-row>
        ...
        <van-cell-group inset>
          <van-cell icon="volume-o">
            <van-tag plain type="primary" size="large">详情</van-tag>
          </van-cell>
          <van-cell>
            <div>
              {{ goodsInfo.content }}
        ...
        <van-cell-group inset>
          <van-cell style="width: 100%; padding: 5px 5px;">
            <van-tabs @click="onClickComment">
              <van-tab>
                <template #title>
                  <van-icon name="exchange" size="19" /> 交易方式
                </template>
              ...
              <van-tab>
                <template #title>
                  <van-icon name="smile-comment-o" size="19" />
                  留言
                </template>

                <div style="margin: 10px 0">
                  <div
                    style="border-bottom: 1px solid #409EFF; padding: 10px 0;"
                  >
                    <van-tag type="primary" size="large">留言内容 ：</van-tag>
                  </div>
                  ...
                  <div class="pd-10" style="text-align: right">
                    <van-button type="info" size="mini" @click="save"
                      >留言</van-button
                    >
                  </div>
                </div>
                ...
                <!--      评论列表-->
                <div>
                  <span v-if="commentInfo.length === 0"
                    >暂时无留言~赶紧抢沙发！</span
                  >
                  <div
                  ...
                  >
                    <div style="display: flex">
                      <!--  头像-->
                      <div
                        style="width: 40px; text-align: center; margin-left: 1px"
                      >
                        <img
                          :src="img_url + item.avatarUrl"
                          style="width: 40px; height: 40px; border-radius: 50%;"
                        />
                      </div>
                      <div
                        style="flex: 1; font-size: 14px; padding: 5px 0; line-height: 25px; margin-left: 10px"
                      >
                        <b style="color: #409EFF">{{ item.nickname }}</b>
                        <div>{{ item.content }}</div>

                        <div
                          style="display: flex; line-height: 20px; margin-top: 5px"
                        >
                    ...
                    <div v-if="item.children.length">
                      <div
                        ...
                        :key="subItem.id"
                        style="background-color: #f0f0f0; padding: 5px 5px; margin-left: 15px; "
                      >
                        <!--          回复列表-->
                        <div
                        ...
                        >
                          <div style="padding-left: 5px">
                            <b style="color: #409EFF">{{ subItem.nickname }}</b>
                            回复
                            <b style="color: #409EFF" v-if="subItem.nickname">{{
                              subItem.replyTo
                            }}</b>
                          ...
                          <div
                            style="display: flex; line-height: 20px; margin-top: 5px; padding-left: 5px"
                          >
                            <div style="width: 180px;">
                              <van-icon name="clock-o"></van-icon
                              ><span style="margin-left: 5px; color: #ccc;">{{
                            ...
                            <div style="text-align: right; flex: 1;">
                              <van-button
                                ...
                                style="margin-left: 5px"
                                @click="
                                  handleReply(subItem.id, subItem.nickname)
                                "
                                v-if="user.id != subItem.studentId"
                                >回复</van-button
                              >
                              <van-button
                                icon-position="left"
...
export default {
  ...
  methods: {
    save() {
      if (
        this.commentForm.contentReply != null ||
